CREATE TABLE `prototype-3_candidate_profiles_to_projects` (
	...
	FOREIGN KEY (`candidate_id`) REFERENCES `prototype-3_candidate_profile`(`userId`) ON UPDATE no action ON DELETE cascade,
...
CREATE TABLE `prototype-3_projectSubmission` (
	...
	FOREIGN KEY (`reviewedBy`) REFERENCES `prototype-3_admin_profile`(`userId`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
...
CREATE TABLE `prototype-3_recruiters_candidates` (
	...
	FOREIGN KEY (`recruiter_id`) REFERENCES `prototype-3_recruiter_profile`(`userId`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`candidate_id`) REFERENCES `prototype-3_candidate_profile`(`userId`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
...
ALTER TABLE `prototype-3_user` ADD `githubUsername` text(255);--> statement-breakpoint
