...
export default function ProjectSubmission({ submission }: { submission: ProjectSubmission }) {
    ...
    return (
        <div
            key={submission.id}
            className="flex items-center justify-between p-4 rounded-lg bg-neutral-100 dark:bg-zinc-800/40"
        >
            <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                    <h4 className="font-medium text-foreground">{submission.projectInstance.project.title + ' ⋅ ' + submission.projectInstance.teamName}</h4>
                    ...
                    <div className="group flex flex-row-reverse items-center">
                        {Array.from({ length: 10 }).map((_, index) => (
                            <button
                                ...
                                className={cn('box-content cursor-pointer w-2.5 flex-0 peer hover:fill-primary peer-hover:fill-primary hover:text-primary peer-hover:text-primary text-muted dark:text-muted-foreground pl-1', index % 2 == 0 && 'rotate-y-180 pl-0', myRating >= 10 - index ? 'text-primary fill-primary group-hover:fill-none' : '')}
                            >
