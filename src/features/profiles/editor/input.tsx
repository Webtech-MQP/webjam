...
export function Input(props: Props) {
    ...
    return (
        <div className="relative">
            ...
            {rest.label && <label className="text-muted dark:text-muted-foreground bg-background absolute top-0 left-2 block -translate-y-1/2 px-2 text-sm font-semibold">{props.label}</label>}
            {icon && <div className="text-muted dark:text-muted-foreground absolute top-1/2 left-2 block -translate-y-1/2 px-2">{icon}</div>}
        </div>
    );
}
